...
If the plan file carries a `> Kanban: <task-id>` line, claim it with `hermes kanban comment <id> "execution started via /do"`. If it has no kanban id, create a card per the `session-kanban` skill.
...
Run `hermes kanban complete <task-id> --result done --summary "<phases shipped + verification evidence>"` per the `session-kanban` skill.

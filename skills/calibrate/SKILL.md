...
- **Memory** — two systems coexist. **Hindsight** (bank `hermes`, loopback :8888) captures sessions automatically via SessionStart/Stop hooks — claude-mem was retired 2026-07-25 (history imported into the bank; DB kept at `~/.claude-mem/` as cold archive). The file-based store at `<config-dir>/projects/<project-slug>/memory/` — the config dir is **per account**: `~/.claude` or `~/.claude-acct2`; use the running session's own paths — is written by hand (one fact per file + a pointer line in its `MEMORY.md` index) and is where durable *preferences and feedback rules* belong. Only suggest an explicit memory write for those — not for facts Hindsight already retains automatically. Second-account wiring parity: [[SOP-07 Second Claude Account Parity]].

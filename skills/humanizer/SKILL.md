...
name: humanizer
description: |
  Remove signs of AI-generated writing from text, or detect them without
  rewriting. Use when editing or reviewing text to make it sound more natural
  and human-written, or when the user asks whether writing reads as AI. Based
  on Wikipedia's comprehensive "Signs of AI writing" guide, merged with
  petergyang/no-ai-slop editing principles. Detects and fixes patterns
  ...
  words, passive voice, negative parallelisms, colon reveals, faux-insight
  setups, fake-profound kickers, and filler phrases.
license: MIT
metadata:
  version: "3.0.0"
  upstream:
    - https://github.com/blader/humanizer (v2.9.1)
...
5. **Make the minimum effective edit** - Fix the AI patterns, errors, and unclear passages; leave strong human sentences alone. The amount of cutting should be proportional to the actual slop. Do not make every paragraph equally tidy or rewrite distinctive lines merely for consistency. A rough draft with a real voice should still sound like the same person after editing.
...
**Also banned outright:** leverage, utilize, facilitate, empower, streamline, robust, cutting-edge, paradigm shift, game changer, this is huge, this changes everything, realm, beacon, multifaceted, meticulous, paramount, transformative, elevate, embark, supercharge, harness, ever-evolving
...
**Signs to watch:** A noun phrase, a colon, then a lowercase dramatic reveal.
**Problem:** LLMs use colons to manufacture drama: "The detail that makes it work: a separate agent grades it." "The best part: it learns." Use colons for lists, labels, and quotes, not fake reveals. Prefer sentence case after a colon unless grammar, a proper noun, a title, or code requires otherwise.
**Before:**
> The detail that makes it work: a separate agent grades it. The best part: it learns.
**After:**
> A separate agent does the grading, which is what makes it work. It also learns over time.
...
**Problem:** These flatter the writer as the lone expert before delivering an ordinary claim. Cut the setup and make the claim stand on its own.
**Before:**
> The part everyone misses: distribution is the real moat.
**After:**
> Distribution is the moat.

### 36. Fake-Profound Kickers

**Problem:** A final "deep" line that turns the point into a cute metaphor, aphorism, or mic-drop sentence. Delete it; do not rewrite it into a better metaphor and do not preserve the rhythm. End on the clearest concrete sentence already in the draft. If the ending needs more closure, add a plain takeaway or next action.
**Before:**
> In the end, the tool doesn't just save time. It gives you your attention back. And attention, it turns out, is the only currency that matters.
**After:**
> The tool cut review time from 30 minutes to 8. (End there, or name the next step.)

### 37. Bullets Where Prose Reads Better

**Problem:** Bullet lists where two sentences of prose would read better, and headers over one- or two-sentence sections. Format should follow the content, not decorate it. Related to §16 (inline-header lists) but applies to any list whose items are really one connected thought.
**Before:**
> ## Why it matters
> - It saves time
> - It reduces errors
**After:**
> The change saves time and reduces errors.
...
**Detect mode.** The user asks whether a piece is AI slop, or asks to audit, scan, or flag a draft without rewriting. Name each pattern from this skill that appears, quote the offending line, and give the fix in a few words. Do not rewrite, do not score the draft, and do not guess whether AI wrote it. AI detectors guess; named patterns are evidence the user can check. Apply the false-positive rules under Detection Guidance: flag clusters, not isolated tells. Offer to edit the draft after.
...
5. Check the final rewrite against `eval.md` in this skill's directory. Answer each check pass or fail. Any fail means the draft isn't done: fix it and re-check. Do this yourself; no separate evaluator agent.

In pasted-text mode, deliver the draft, the brief "still-AI" bullets, the final rewrite, and a short **What changed** section. In file and embedded modes, run the same loop but deliver only what the mode calls for (see Invocation Modes). In detect mode, skip the loop and deliver only the findings report.

## Reference

This skill is based on [Wikipedia:Signs of AI writing](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing), maintained by WikiProject AI Cleanup, via [blader/humanizer](https://github.com/blader/humanizer) v2.9.1. The patterns documented there come from observations of thousands of instances of AI-generated text on Wikipedia.

Detect mode, the minimum-effective-edit principle, the eval checklist, and patterns 34-37 are merged from [petergyang/no-ai-slop](https://github.com/petergyang/no-ai-slop) (MIT).

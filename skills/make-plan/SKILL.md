...
### Kanban registration (before Phase 0)

Follow the `session-kanban` skill to create the board card for this plan, using idempotency key `plan:<plan-slug>`. Record the returned task id at the top of the plan file as `> Kanban: <task-id>`.
...
4. On plan completion, run `hermes kanban complete <task-id> --summary` per the `session-kanban` skill.
